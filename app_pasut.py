...
with st.sidebar:
    if os.path.exists(NAMA_FILE_LOGO):
        ...
        st.markdown(
            f"""
            <div style="display: flex; justify-content: center; align-items: center; width: 100%; margin-top: 20px; margin-bottom: 10px;">
    ...
    st.markdown("<p style='text-align: center; color: #1e3a8a; margin-top: 10px; font-size: 0.85rem; font-weight: bold;'>STASIUN METEOROLOGI MARITIM TANJUNG PRIOK</p>", unsafe_allow_html=True)
    st.divider()
...
if df_pred is not None and not df_pred.empty:
    ...
    selisih_tren = (df_pred.loc[(df_pred[col_tgl] - (sekarang_naive + timedelta(hours=3))).abs().idxmin(), col_val]) - h_now
    icon_t, status_t = ("📈", "NAIK") if selisih_tren > 0.05 else ("📉", "TURUN") if selisih_tren < -0.05 else ("↔️", "STAGNAN")
    m4.metric("Tren (3j Kedepan)", f"{icon_t} {status_t}")
    ...
    if not df_plot.empty:
        ...
        fig.add_hline(y=2.5, line_dash="dash", line_color="#ef4444", annotation_text="🚨 AWAS ROB", annotation_position="top right")
        fig.add_hline(y=2.3, line_dash="dash", line_color="#ea580c", annotation_text="📢 WASPADA ROB", annotation_position="top right")
