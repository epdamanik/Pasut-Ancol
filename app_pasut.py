...
if df_pred is not None and not df_pred.empty:
    ...
    # Ambil nilai MAX & MIN hari ini + Jam-nya
    df_h = df_pred[df_pred[col_tgl].dt.date == sekarang.date()]
    if not df_h.empty:
        idx_max = df_h[col_val].idxmax()
        idx_min = df_h[col_val].idxmin()
        v_max_today, t_max_today = df_h.loc[idx_max, col_val], df_h.loc[idx_max, col_tgl].strftime("%H:%M")
        v_min_today, t_min_today = df_h.loc[idx_min, col_val], df_h.loc[idx_min, col_tgl].strftime("%H:%M")
        
        st.markdown(f'<div class="summary-box"><span class="summary-text">📅 {sekarang.strftime("%d %b %Y")} | <span style="color: #ef4444;">▲ MAX: {v_max_today:.2f}m ({t_max_today})</span> | <span style="color: #3b82f6;">▼ MIN: {v_min_today:.2f}m ({t_min_today})</span></span></div>', unsafe_allow_html=True)
    ...
    m2.metric("AWS Tj. Priok", f"{live_data['aws']:.2f} m" if live_data["aws"] else "N/A", delta=f"({selisih_aws:+.2f}m)" if selisih_aws is not None else None)
    m3.metric("TMA Psr. Ikan", f"{live_data['bpbd']:.2f} m" if live_data["bpbd"] else "N/A", delta=f"({selisih_bpbd:+.2f}m)" if selisih_bpbd is not None else None)
    
    h_next = df_pred.loc[(df_pred[col_tgl] - (sekarang_naive + timedelta(hours=3))).abs().idxmin(), col_val]
    selisih_tren = h_next - h_now
    icon, status = ("📈", "NAIK") if selisih_tren > 0.05 else ("📉", "TURUN") if selisih_tren < -0.05 else ("↔️", "STAGNAN")
    ...
    if not df_plot.empty:
        ...
        for day in unique_days:
            ...
            idx_max_day = df_day[col_val].idxmax()
            idx_min_day = df_day[col_val].idxmin()
            
            fig.add_trace(go.Scatter(
                x=[df_day.loc[idx_max_day, col_tgl]], y=[df_day.loc[idx_max_day, col_val]],
                mode='markers+text', name=f'Max {day}',
                marker=dict(color='#ef4444', size=8, symbol='circle'),
                text=[f"{df_day.loc[idx_max_day, col_val]:.2f}"],
                textposition="top center", showlegend=False
            ))
            
            fig.add_trace(go.Scatter(
                x=[df_day.loc[idx_min_day, col_tgl]], y=[df_day.loc[idx_min_day, col_val]],
                mode='markers+text', name=f'Min {day}',
                marker=dict(color='#3b82f6', size=8, symbol='circle'),
                text=[f"{df_day.loc[idx_min_day, col_val]:.2f}"],
    ...
    with c1: st.download_button("📥 Scrape AWS Maritim Tj. Priok CSV", open(FILE_HISTORY_AWS, 'rb') if os.path.exists(FILE_HISTORY_AWS) else b"", "AWS.csv", use_container_width=True)
    with c2: st.download_button("📥 Scrape Pintu air Pasar Ikan I CSV", open(FILE_HISTORY_BPBD, 'rb') if os.path.exists(FILE_HISTORY_BPBD) else b"", "Pasarikan.csv", use_container_width=True)
    with c3: 
