...
st.markdown("""
    ...
    /* 1. Bagian merapatkan header: Margin-bottom dikecilkan */
    .header-text { 
        text-align: center; 
        width: 100%; 
        margin-top: -15px; 
        margin-bottom: 0px !important; /* Dibuat 0 agar tidak ada jarak ke bawah */
    ...
    /* 2. Bagian merapatkan summary box ke header: margin-top negatif */
    .summary-box {
        background-color: #f1f5f9 !important; 
        padding: 8px !important; 
        border-radius: 10px !important; 
        margin-top: -15px !important; /* Nilai negatif menarik kotak ke atas menuju header */
        margin-bottom: 10px !important; 
...
if df_pred is not None and not df_pred.empty:
    ...
    m4.metric("Tren (3j Kedepan)", f"{icon} {'NAIK' if selisih > 0.05 else 'TURUN' if selisih < -0.05 else 'STAGNAN'}")
    ...
    if not df_plot.empty:
        ...
        for file, label, color in [(FILE_HISTORY_AWS, 'AWS (Hist)', '#7c3aed'), (FILE_HISTORY_BPBD, 'Psr. Ikan (Hist)', '#f59e0b')]:
            if os.path.exists(file):
                ...
                if not dh.empty:
                    fig.add_trace(go.Scatter(x=dh['waktu'], y=dh['nilai'], name=label, line=dict(color=color, width=3.5, shape='spline')))
